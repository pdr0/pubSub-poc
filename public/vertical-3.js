...
var container = document.querySelector('#vertical-3');

var onMessage = function onMessage(msg, data) {
  console.log("TOPIC: ".concat(msg, " - payload: ").concat(data));

  if (msg === _topics["default"]["lotto.main.help.click"]) {
    container.className = '';
    container.classList.add('on-help');
  }

  if (msg === _topics["default"]["lotto.main.login.click"]) {
    container.className = '';
  ...
  if (msg === _topics["default"]["lotto.container-4.click"]) {
    container.className = '';
...
container.addEventListener('click', function (evt) {
  window.messages.publishSync(_topics["default"]["lotto.container-3.click"], true);
});
window.messages.subscribe(_topics["default"]["lotto.main.header.expanded"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.container-1.click"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.container-2.click"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.container-3.click"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.container-4.click"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.main.help.click"], onMessage);
window.messages.subscribe(_topics["default"]["lotto.main.login.click"], onMessage);
},{"../topics":1}]},{},[2]);
